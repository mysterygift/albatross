...
Form: Account (required; postable only), Description (required), Estimated cost, Actual cost, Vendor (optional). New line items default to the **Allow** classification with a matching details row. Inline form uses the current leaf account. Submit creates the line item and closes the dialog or clears the inline target. **Delete** — Each expanded line-item row has a trash icon next to Examine. If the line item has no matched expenses or petty-cash floats, a confirmation dialog deletes it. If it has associations, the dialog lists each matched expense and float with a dropdown to choose another line item (same account first, otherwise any other line item in the production); on confirm, matches and floats are relocated, matched expenses are recoded to the target line item&apos;s account, then the line item is soft-deleted. **Edit** — Use Examine line item → Edit in the detail sheet (`LineItemDetailPanel`).
