...
PEOPLE
1. Fix CrewDetailPage/CastDetailPage returning user back to Person page when clicking on the back arrow, should return user to Crew/Cast page respectively.
...
BIG PICTURE FEATURE IDEAS:
Schedule & Budget Versioning
Tutorial State
Encrypt Production DB + Login
...
Check performance on local server.
sell out and launch a SaaS /s